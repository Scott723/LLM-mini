...
from myqwen.modeling.attention.full_attention import KVCache
...
class DecoderModel(nn.Module):
    ...
    def _prepare_past_key_values(
        ...
        if len(past_key_values) != len(self.layers):
            raise ValueError(
                f"Expected {len(self.layers)} layer caches, got {len(past_key_values)}"
            )
    ...
    def forward(
        ...
        if attention_mask is not None and attention_mask.shape != (batch_size, total_length):
            raise ValueError(
                f"attention_mask must have shape {(batch_size, total_length)}, got {tuple(attention_mask.shape)}"
            )
